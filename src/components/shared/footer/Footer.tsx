...
const Footer = () => {
  const footerLinks = {
    ...
    Resources: [
      ...
      { name: "GitHub", href: "https://github.com/Rasel-231" },
      { name: "Support", href: "#" },
    ],
  };
  ...
  return (
    <footer className="relative w-full bg-slate-950 text-white border-t border-white/10 overflow-hidden">
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-12 lg:gap-8">
          ...
          <div className="col-span-full lg:col-span-2 space-y-6">
            <Link href="/" className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-lg text-white">R</div>
              <span className="text-2xl font-bold tracking-tight">RaselHub</span>
            </Link>
            <p className="text-slate-400 text-sm max-w-xs leading-relaxed">
              Building high-performance digital products with passion. Let&apos;s create something extraordinary together.
            </p>
          </div>

          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title} className="col-span-1">
              <h4 className="text-sm font-semibold text-white mb-6 uppercase tracking-wider">{title}</h4>
              <ul className="space-y-4">
                {links.map((link) => (
                  <li key={link.name}>
                    <a href={link.href} className="text-slate-400 hover:text-indigo-400 transition-colors text-sm">
                      {link.name}
                    </a>
                  </li>
                ))}
        ...
        <div className="mt-16 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-slate-500 text-xs">© {new Date().getFullYear()} RaselHub. Built with Next.js & Tailwind.</p>
          <div className="flex gap-4">
            {socialLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-indigo-600 hover:border-indigo-500 transition-all text-white"
              >
                {link.icon}
              </a>
            ))}
