...
// ১. কম্পোনেন্টটি Navbar-এর বাইরে ডিক্লেয়ার করা হয়েছে (এরর ফিক্সড)
...
const Navbar = () => {
  ...
  const navItems = useMemo((): NavItem[] => {
    ...
    if (profile) {
      return [...baseItems, { name: "Panel", icon: <UserStar size={18} />, href: "/dashboard" }, { name: "Logout", icon: <LogOut size={18} />, href: "#", onClick: handleLogout }];
    }
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? "py-3 bg-slate-950/80 backdrop-blur-md border-b border-white/5 shadow-lg" : "py-5 bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-xl font-black text-white"><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Rasel</span>Hub</span>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-6 bg-slate-900/20 px-6 py-2 rounded-full border border-white/5 backdrop-blur-sm">
            {navItems.map((item) => (
              <Link key={item.name} href={item.href} onClick={item.onClick} className="flex items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-indigo-400 transition-colors">
                {item.icon} {item.name}
              </Link>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute w-full bg-slate-950/95 border-b border-white/5 backdrop-blur-lg p-4 space-y-4">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} onClick={(e) => { if (item.onClick) item.onClick(e); setIsOpen(false); }} className="flex items-center gap-4 px-4 py-3 text-slate-300 hover:bg-white/5 rounded-xl transition-all">
              {item.icon} {item.name}
            </Link>
          ))}
          <div className="px-4">
